...
export async function POST(request: Request) {
  ...
  const parsed = trackSchema.safeParse((body as { track?: unknown } | undefined)?.track);
  if (!parsed.success) return badRequest('A complete track is required');
  try {
    const result = await ensureTrackReady(parsed.data);
    await recordListeningEvent({ id: `event:${crypto.randomUUID()}`, trackId: parsed.data.id, eventType: 'play', context: 'manual' }).catch(() => undefined);
