...
export function SearchView({ recentSearches, onPlay, onLike, onAdd }: SearchViewProps) {
  ...
  return (
    <div className="view search-view">
      ...
      {query && <div className="search-results"><div className="catalog-note">Global catalog · local availability shown on play</div>{loading && <div className="loading-line">Searching the world of music…</div>}{!loading && results && <><ResultGroup title="Songs">{results.tracks.length ? <div className="track-list">{results.tracks.map((track) => <TrackRow key={track.id} track={track} onPlay={onPlay} onLike={onLike} onAdd={onAdd} status={track.isLocal ? 'ready' : 'preparing'} />)}</div> : <div className="empty-state">No songs found yet.</div>}</ResultGroup><SearchPeople title="Artists" items={results.artists} /><SearchAlbums title="Albums" items={results.albums} /></>}</div>}
    </div>
  );
}

function SearchPeople({ title, items }: { title: string; items: Artist[] }) {
  if (!items.length) return null;
  return <ResultGroup title={title}><div className="artist-scroll">{items.map((artist) => <button className="artist-card" key={artist.id}><Artwork src={artist.imageUrl} alt="" size="md" rounded="circle" /><strong>{artist.name}</strong><span>Artist</span></button>)}</div></ResultGroup>;
}

function SearchAlbums({ title, items }: { title: string; items: Album[] }) {
  if (!items.length) return null;
  return <ResultGroup title={title}><div className="artwork-scroll">{items.map((album) => <button className="artwork-card" key={album.id}><Artwork src={album.artworkUrl} alt="" size="md" /><strong>{album.title}</strong><span>{album.artistName}</span></button>)}</div></ResultGroup>;
}
