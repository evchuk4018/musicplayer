import { ChevronDown, ChevronRight, ChevronUp, ListMusic, Pencil, Plus, Trash2 } from 'lucide-react';
...
export function LibraryView({ library, onPlay, onPlayAll, onLike, onAdd, onSave, onCreatePlaylist, onRenamePlaylist, onDeletePlaylist, onChangeArtwork, onRemoveTrack, onMoveTrack, selectedPlaylist, onSelectPlaylist, onClosePlaylist }: LibraryViewProps) {
  const [filter, setFilter] = useState<'all' | 'playlists' | 'songs'>('all');
  if (selectedPlaylist) {
    return <PlaylistDetail playlist={selectedPlaylist} onBack={onClosePlaylist} onPlay={onPlay} onPlayAll={onPlayAll} onLike={onLike} onAdd={onAdd} onSave={onSave} onRename={() => onRenamePlaylist(selectedPlaylist)} onChangeArtwork={() => onChangeArtwork(selectedPlaylist)} onDelete={() => { onDeletePlaylist(selectedPlaylist); onClosePlaylist(); }} onRemoveTrack={onRemoveTrack} onMoveTrack={onMoveTrack} />;
  }
  return (
