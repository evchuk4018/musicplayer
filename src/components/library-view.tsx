...
export function LibraryView({ library, onPlay, onPlayAll, onLike, onAdd, onCreatePlaylist, onRenamePlaylist, onDeletePlaylist, onRemoveTrack, selectedPlaylist, onSelectPlaylist, onClosePlaylist }: LibraryViewProps) {
  const [filter, setFilter] = useState<'all' | 'playlists' | 'songs'>('all');
  if (selectedPlaylist) {
    return <PlaylistDetail playlist={selectedPlaylist} onBack={onClosePlaylist} onPlay={onPlay} onPlayAll={onPlayAll} onLike={onLike} onAdd={onAdd} onRename={() => onRenamePlaylist(selectedPlaylist)} onDelete={() => { onDeletePlaylist(selectedPlaylist); onClosePlaylist(); }} onRemoveTrack={onRemoveTrack} />;
  }
  return (
