import { Bookmark, ChevronDown, Heart, ListMusic, Pause, Play, Radio, Repeat2, Shuffle, SkipBack, SkipForward, Volume2 } from 'lucide-react';
...
export function NowPlaying({ track, queue, isPlaying, progressSeconds, durationSeconds, isLiked, isSaved, shuffle, repeat, onClose, onToggle, onPrevious, onNext, onSeek, onShuffle, onRepeat, onLike, onSave, onAdd, volume, onVolume, autoplayEnabled, onAutoplay, onQueueTrack, onRadio }: NowPlayingProps) {
  return (
    <div className="now-playing-screen">
      ...
      <main className="now-playing-content">
        ...
        <div className="now-playing-meta"><div><h1>{track.title}</h1><p>{track.artistName}</p></div><div className="now-meta-actions"><button className={`icon-button now-like ${isLiked ? 'is-liked' : ''}`} onClick={onLike} aria-label={isLiked ? 'Unlike song' : 'Like song'}><Heart size={26} fill={isLiked ? 'currentColor' : 'none'} /></button><button className={`icon-button now-like ${isSaved ? 'is-saved' : ''}`} onClick={onSave} aria-label={isSaved ? 'Unsave song' : 'Save song'}><Bookmark size={24} fill={isSaved ? 'currentColor' : 'none'} /></button></div></div>
        ...
        <div className="now-actions"><button className="secondary-action" onClick={onAdd}><ListMusic size={18} /> Add to playlist</button><button className="secondary-action" onClick={onRadio}><Radio size={17} /> Radio</button><button className={`secondary-action autoplay-action ${autoplayEnabled ? 'is-active' : ''}`} onClick={onAutoplay}>Autoplay {autoplayEnabled ? 'on' : 'off'}</button><label className="volume-control"><Volume2 size={18} /><input type="range" min="0" max="1" step="0.01" value={volume} onChange={(event) => onVolume(Number(event.target.value))} aria-label="Volume" /></label></div>
